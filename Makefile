...
requirements_slurm:
	srun  --partition=standard --pty -c 1 --mem=7g --time=00:30:00 bash -c "conda env update --name $(PROJECT_NAME) --file environment.yml --prune -v"
...
## Run additional_zarr_filtering.py using SLURM batch jobs
.PHONY: slurm_additional_zarr_filtering
slurm_additional_zarr_filtering:
	@config_file=$$($(PYTHON_INTERPRETER) scripts/get_config_value.py --section slurm_additional_zarr_filtering --key txt_file); \
	if [ -z "$$config_file" ]; then \
		echo "Error: txt_file not found in [slurm_additional_zarr_filtering] section of config.yaml"; \
		exit 1; \
	fi; \
	if [ ! -f "config/$$config_file" ]; then \
		echo "Error: File config/$$config_file does not exist!"; \
	...
	input_path="$$(realpath config/$$config_file)"; \
	base_name="$$(basename $$config_file .txt)"; \
	output_path="data/additional_filtering/$${base_name}_additional_filtered.zarr"; \
	echo "Using config file from config.yaml: config/$$config_file"; \
	echo "Output will be saved to: $$output_path"; \
	slurm_account=$$($(PYTHON_INTERPRETER) scripts/get_config_value.py --section SLURM --key slurm_account --fallback ""); \
	if [ -z "$$slurm_account" ]; then \
	...
	if ! $(PYTHON_INTERPRETER) scripts/get_config_value.py --section slurm_additional_zarr_filtering --check-required cpus-per-task mem-per-cpu time >/dev/null 2>&1; then \
		echo "Error: Missing SLURM parameters in [slurm_additional_zarr_filtering] section of config.yaml"; \
		exit 1; \
	fi; \
	cpus_per_task=$$($(PYTHON_INTERPRETER) scripts/get_config_value.py --section slurm_additional_zarr_filtering --key cpus-per-task); \
	mem_per_cpu=$$($(PYTHON_INTERPRETER) scripts/get_config_value.py --section slurm_additional_zarr_filtering --key mem-per-cpu); \
	time=$$($(PYTHON_INTERPRETER) scripts/get_config_value.py --section slurm_additional_zarr_filtering --key time); \
	echo "Using SLURM parameters: CPUs=$$cpus_per_task, Memory=$$mem_per_cpu per CPU, Time=$$time"; \
	app_flags=$$($(PYTHON_INTERPRETER) scripts/get_config_value.py --section slurm_additional_zarr_filtering --get-flags); \
	echo "Using application flags: $$app_flags"; \
	mkdir -p data/additional_filtering logs; \
	if [ -f "$$input_path" ] && [ "$${input_path##*.}" = "txt" ]; then \
		echo "Processing Zarr files listed in: $$input_path"; \
		job_ids=(); \
		while IFS= read -r zarr_file || [ -n "$$zarr_file" ]; do \
			if [ -n "$$zarr_file" ] && [ -d "$$zarr_file" ]; then \
				echo "Submitting SLURM job for: $$zarr_file"; \
				job_name="additional_zarr_filtering_$$(basename $$zarr_file .zarr)"; \
				log_file="logs/$${job_name}.slurm.out"; \
				if [ -n "$$slurm_account" ]; then \
					jobid=$$(sbatch --parsable --account="$$slurm_account" --time="$$time" --cpus-per-task="$$cpus_per_task" --mem-per-cpu="$$mem_per_cpu" \
						--job-name="$$job_name" --output="$$log_file" \
						--wrap="source ~/.bashrc && conda activate $(PROJECT_NAME) && $(PYTHON_INTERPRETER) post_varloc_data_pipeline/additional_zarr_filtering.py --input '$$zarr_file' $$app_flags"); \
				else \
					jobid=$$(sbatch --parsable --time="$$time" --cpus-per-task="$$cpus_per_task" --mem-per-cpu="$$mem_per_cpu" \
						--job-name="$$job_name" --output="$$log_file" \
						--wrap="source ~/.bashrc && conda activate $(PROJECT_NAME) && $(PYTHON_INTERPRETER) post_varloc_data_pipeline/additional_zarr_filtering.py --input '$$zarr_file' $$app_flags"); \
		...
		if [ $${#job_ids[@]} -gt 0 ]; then \
			...
			echo "All additional zarr filtering jobs completed successfully!"; \
		fi; \
	elif [ -d "$$input_path" ]; then \
		echo "Submitting SLURM job for single Zarr file: $$input_path"; \
		job_name="additional_zarr_filtering_$$(basename $$input_path .zarr)"; \
		log_file="logs/$${job_name}.slurm.out"; \
		if [ -n "$$slurm_account" ]; then \
			jobid=$$(sbatch --parsable --account="$$slurm_account" --time="$$time" --cpus-per-task="$$cpus_per_task" --mem-per-cpu="$$mem_per_cpu" \
				--job-name="$$job_name" --output="$$log_file" \
				--wrap="source ~/.bashrc && conda activate $(PROJECT_NAME) && $(PYTHON_INTERPRETER) post_varloc_data_pipeline/additional_zarr_filtering.py --input '$$input_path' $$app_flags"); \
		else \
			jobid=$$(sbatch --parsable --time="$$time" --cpus-per-task="$$cpus_per_task" --mem-per-cpu="$$mem_per_cpu" \
				--job-name="$$job_name" --output="$$log_file" \
				--wrap="source ~/.bashrc && conda activate $(PROJECT_NAME) && $(PYTHON_INTERPRETER) post_varloc_data_pipeline/additional_zarr_filtering.py --input '$$input_path' $$app_flags"); \
