...
def chromosome_filter(xr_df: xr.Dataset, chromosome: str):
    # create a mask for the chromosome
    mask = xr_df["chromosome"] == chromosome
    # return the xarray dataset where the mask is false
    return xr_df.where(~mask, drop=True)
...
def groupby_genome_location(xr_df: xr.Dataset):
    ...
    additional = [
        ...
        "max_af",
        "variant_class",
        # "var_type", can be both exon and noncoding
        ]
...
def main():
    ...
    # print the data type of xr_df
    print("the data type of xr_df is: ", type(xr_df))
    # print the index of xr_df
    print("the index of xr_df is: ", xr_df.indexes)
    print("xr_df size: ", xr_df.info)
    xr_df = xr_df.isel(indexers={'index': slice(0, 50000)})
    print("xr_df size: ", xr_df.info)
    ...
    # print("complete impact filter.  the time is now: ", datetime.datetime.now())
    # get the number of lines in the filtered_xr_df
    # print("filtered_xr_df size: ", xr_df.info)

    # filter by chromosome 1
    # print("running chromosome filter.  the time is now: ", datetime.datetime.now())
    # xr_df = chromosome_filter(xr_df, chromosome="1")
    # print("complete chromosome filter.  the time is now: ", datetime.datetime.now())
    # print("filtered_xr_df size: ", xr_df.info)
    ...
    print("filtered_xr_df size before groupby: ", xr_df.info)
    ...
    xr_df = groupby_genome_location(xr_df)
     # print the number of rows in the filtered_xr_df
    print("filtered_xr_df size after groupby: ", xr_df.info)

    # get all the column names
    columns = xr_df.columns
    # if the column ends in unique, then remove that string form the end.  for example 'observationsunique' will now be 'observations'
    columns = [column.replace("unique", "") for column in columns]
    # set the columns to the new column names
    ...
    # print the number of rows in the filtered_xr_df
    print("filtered_xr_df size after saving: ", xr_df.info)
    """
    ...
    logger.info(f"number of rows in filtered_xr_df at end: {xr_df.info}")
