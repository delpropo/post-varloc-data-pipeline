...
def exon_filter(xr_df: xr.Dataset):
    ...
    xr_df = xarray_filter_max_af(xr_df, max_af_ceiling=0.01)
    # filter the xarray for protein
    xr_df = xarray_filter_var_type(xr_df, filter_type="exon")
...
def noncoding_impact_filter(xr_df: xr.Dataset):
    ...
    xr_df = xarray_filter_max_af(xr_df, max_af_ceiling=0.01)
    # filter the xarray for protein
    xr_df = xarray_filter_var_type(xr_df, filter_type="noncoding")
    #xr_df = xarray_filter_impact(xr_df, impact=["HIGH", "MODERATE"])
...
def main():
    ...
    # get the number of lines in the xr_df
    ...
    # save the xr_df to a tsv file called filter_test_deleteme.tsv
    #print("saving xr_df to a tsv file.  the time is now: ", datetime.datetime.now())
    #xr_df.to_csv("filter_test_deleteme.tsv", sep="\t")


    #exon_file_name = "exon_filter_" + output_file
    #xarray_to_csv(exon_xr_df, file_name=exon_file_name)
    #print(f"impact filter file saved as {exon_file_name}")
    #logger.info(f"impact filter file saved as {exon_file_name}")


    # print the number of rows in the xr_df
    print("xr_df size before groupby: ", xr_df.info)
    # save the xr_df to a tsv file

    print("started groupby_genome_location function ", datetime.datetime.now())
    xr_df = groupby_genome_location(xr_df)
     # print the number of rows in the xr_df
    print("xr_df size after groupby: ", xr_df.info)
    ...
    # print the number of rows in the xr_df
    print("xr_df size after saving: ", xr_df.info)
    """
    ...
    # noncoding_filter
    noncoding_impactHM_xr_df = noncoding_impact_filter(xr_df)

    # noncoding save to file
    noncoding_impactHM_file_name = "noncoding_impact_filter_" + output_file
    xarray_to_csv(noncoding_impactHM_xr_df, file_name=noncoding_impactHM_file_name)
